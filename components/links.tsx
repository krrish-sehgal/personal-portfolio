...
function NostrIcon(props: React.SVGProps<SVGSVGElement>) {
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden {...props}>
      <path fill="currentColor" d="M12 2l2.4 6.9L22 10l-5.5 4 1.9 7L12 17l-6.4 4 1.9-7L2 10l7.6-1.1z" />
    </svg>
  )
}
...
export function ImportantLinks() {
  ...
  const links = [
    ...
    site.hero?.primaryCta?.url
      ? { label: site.hero.primaryCta.label, href: site.hero.primaryCta.url, Icon: ExternalLinkIcon }
      : null,
    site.writing?.[0]?.link ? { label: "Medium", href: site.writing[0].link, Icon: ExternalLinkIcon } : null,
  ].filter(Boolean) as { label: string; href: string; Icon: React.FC<React.SVGProps<SVGSVGElement>> }[]

  return (
    <div className="w-full max-w-none">
      <h2 className="text-3xl font-bold">Important Links</h2>
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 w-full">
